...
### 附 CPython全局解释器锁
全局解释器锁（Global Interpreter Lock），简称为GIL，是Python中一个比较著名的性能瓶颈，它导致了Python多线程的低效，也在一定程度上加剧了Python速度慢的问题。不过需要明确的是，我们一般说Python有全局解释器锁，但实际上这是不准确的。实际上，只有Python的C语言实现CPython才有全局解释器锁，其他的实现是没有的，但由于CPython是使用最广泛的Python版本，所以大多数人都将CPython与Python划了等号，但这实际上是不准确的。

GIL是CPython解释器用于同步的机制，它使得在任何时刻，解释器只执行一个线程。一个线程想要执行，必须要先得到GIL；在它遇到I/O等操作的时候，它会释放GIL给别的线程抢占。这就导致了即使你使用了多线程、想要进行多线程的并发，而你的设备也支持多线程并发（比如多核CPU），但你只会通过多线程提升你I/O密集型计算的效率，对CPU密集性计算的提升很小。

正如黑格尔所说，存在即合理，在我们使用者看来GIL除了阻碍并发之外没什么用处，但开发者设计它一定是有他们的道理的，或者说是有苦衷的。

开发者引入GIL的本意应该是保证多线程安全。在各大厂商推出多核处理器之后，为了利用多核处理器的性能，出现了多线程的编程方式，这就对编程语言提出了新的要求。Python作为一款一贯适应潮流的语言，自然也引入了多线程的机制。但多线程势必会带来线程间数据与状态一致的问题。对此，CPython的解决方案就是GIL。GIL最根本的目的是防止多个线程同时执行同一段Python字节码。CPython采用了一种所谓“引用计数”的内存管理机制，也就是说，CPython解释器会记录每个资源的被引用次数；如果次数为0，那么说明这个资源已经没用了，就会被释放。这个机制不是内存安全的，当它与CPython的解释执行方式结合，就会出现很严重的问题。比如下面这个例子。A与B两个进程都引用了一个资源，资源的引用计数为2；现在是A在执行，并且正要销毁这个资源（引用计数值减一），恰恰在这时，由于进程调度，A被阻塞了，B开始执行；设想B执行完成了，它也会将引用计数减一，这时引用计数已经变为0，这个资源就会被彻底销毁。那么当A再度被唤醒，它还会继续自己的销毁操作，但这个资源已经被销毁了，这就会导致很多严重的问题。

现在来看，为了应对这一问题，可能会存在更好的解决方法，但在当时而言这种方法已经不错了。尤其是考虑到Python的初衷就是像ABC语言一样让程序员与非程序员都可以使用，这就要求Python不能将线程安全做得太复杂。而在大家发现这种模式的低效后，已经有太多人，包括第三方库的开发者习惯并依赖于这一特性（默认CPython内部是线程安全的，不用自己考虑这一问题），而且大量已经存在的代码也利用了这一特性，所以去除GIL变得太难了。何况GIL也有它的优点，比如它在单线程下更快、更易于包装C语言库进Python等。

在肉眼可见的将来，GIL还将作为CPython的特性保留下去。
